...
*	* Copyright 2008 verlihub-project (www.verlihub-project.org) 2005-08	*	*
*	*-----------------------------------------------------------------------*	*
*											*
...
bool cpiLua::CallAll(char * fncname, char * args[])
{
...
bool cpiLua::OnValidateTag(cConnDC *conn, cDCTag *tag)
{
	...
	{
		char * args[] = {	(char *)conn->mpUser->mNick.c_str(),
					(char *)tag->mTag.c_str(), // string mTag, string mTagBody, double mClientVersion
					NULL };
...
char * cpiLua::toString(int number)
{
	char *buffer = new char[25];
	int n = sprintf (buffer, "%d", number);
	if(n < 0 || n > 25) {
		delete buffer;
