...
/*#else
		...
		if(conn->Log(2)) conn->LogStream() << "Wrong password, ban " << mS->mC.pwd_tmpban <<"s, closing" << endl;
...
int cDCProto::DC_To(cMessageDC * msg, cConnDC * conn)
{
	...
	{
		if(conn->Log(2)) conn->LogStream() << "Pretends to be someone else in PM (" << msg->ChunkString(eCH_PM_FROM) << ")." <<endl;
		conn->CloseNow();
		return -1;
	}
	cTime now;
...
{
	bool WrongIP = true;
	if(WrongIP && (conn->mAddrIP == ip || conn->mAddrIP == "127.0.0.1"))
	{
		WrongIP = false;
	}
...
int cDCProto::DC_ConnectToMe(cMessageDC * msg, cConnDC * conn)
{
	...
	{
		if(conn->mpUser->mShare < mS->mC.min_share_use_hub) 
		{
