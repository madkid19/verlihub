...
/*
casyncconn.cpp: In constructor `nServer::cAsyncConn::cAsyncConn(int,
   nServer::cAsyncSocketServer*, nServer::nEnums::tConnType)':
casyncconn.cpp:86: error: `socklen_t' undeclared (first use this function)
*/
using namespace std;
...
int cAsyncConn::Write(const string &data, bool Flush)
{
	...
	// cout << "Data to send: " << send_buffer << endl;
	if(SendAll(send_buffer,size_sent) == -1)
	{
		// cout << "Got -1 from SendAll" << endl;
		// analyze the error
		...
		if(size_sent > 0)
		{
			...
			if(!appended)
				...
				//mBufSend = data.substr(size_sent,send_size-size_sent);
			// else cut off the beginning of the buffer
			else
				// this makes mBuffSend not grow
				//mBufSend = mBufSend.substr(size_sent, tmp.npos);
				StrCutLeft( mBufSend, size_sent);
				//tmp.assign(mBufSend,size_sent, tmp.npos);
				//mBufSend.reserve(0);
		}
		else
		{
		...
		if(mxServer && ok)
		{
			...
			//mBufSend.reserve(unsigned(mBufSend.size() * 1.2));
		}
	}
...
#if !defined _WIN32
//		inet_aton(host.c_str(), &AddrIN.sin_addr); // override it
#else
//		AddrIN.sin_addr.s_addr = inet_addr(host.c_str());
#endif
	}
	//cout << "Resolving host: " << host << " to " <<AddrIN.sin_addr.s_addr << endl;
